...
class CreateEfrataServicefinalsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('efrata_servicefinals', function (Blueprint $table) {
            $table->id();
            $table->date('date');
            $table->unsignedBigInteger('efrata_antrianservice_id');
            $table->foreign('efrata_antrianservice_id')->references('id')->on('efrata_antrianservices')->onDelete('cascade')->onUpdate('cascade');
    ...
    {
        Schema::dropIfExists('efrata_servicefinals');
    }
}
