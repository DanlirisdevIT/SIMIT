...
class CreateEfrataAntrianservicesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('efrata_antrianservices', function (Blueprint $table) {
            $table->id();
            $table->date('date');
            $table->unsignedBigInteger('efrata_history_id');
            $table->foreign('efrata_history_id')->references('id')->on('efrata_histories')->onDelete('cascade')->onUpdate('cascade');
    ...
    {
        Schema::dropIfExists('efrata_antrianservices');
    }
}
