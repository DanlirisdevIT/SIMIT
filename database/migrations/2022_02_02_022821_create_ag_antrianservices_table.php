...
class CreateAgAntrianservicesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('ag_antrianservices', function (Blueprint $table) {
            $table->id();
            $table->date('date');
            $table->unsignedBigInteger('ag_history_id');
            $table->foreign('ag_history_id')->references('id')->on('ag_histories')->onDelete('cascade')->onUpdate('cascade');
    ...
    {
        Schema::dropIfExists('ag_antrianservices');
    }
}
