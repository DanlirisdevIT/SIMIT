            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <ul id="saveForm_errList"></ul>
                        ...
                        <div class="form-group">
                            <label name="efrata_budget_id" class="col-sm-4 control-label"> Budget </label>
                            <select class="form-control" id="efrata_budget_id" name="efrata_budget_id">
                                <option value=""> -- Pilih Budget -- </option>
                                @foreach ($efrata_budgets as $budget)
            ...
            <div class="modal-content">
                ...
                <form id="updatePemasukanForm" name="updatePemasukanForm" class="form-horizontal">
                    ...
                    <div class="modal-body" style="overflow:hidden;">
                        ...
                        <div class="form-group">
                            <label name="ef_uid" class="col-sm-4 control-label"> EF UID </label>
                            <input type="text" class="form-control" id="ef_uid" name="ef_uid" maxlength="50" readonly>
                        </div>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            var table = $('.datatables').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('efrata_pemasukan.index') }}",
                method: 'GET',
                columns: [
                    {data: 'DT_RowIndex', name: 'DT_RowIndex', width: '5%'},
                    {data: 'date', name: 'date', width: '15%'},
                    {data: 'ef_uid', name: 'ef_uid', width: '15%'},
        ...
        $(document).ready(function () {
            ...
            $.ajax({
                url: "{{ route('efrata_pemasukan.create') }}",
                type: "GET",
                dataType: "json",
                success: function(response){
                    ...
                    $('#pemasukan_type').on('change', function() {
                        ...
                        dropdown = '<label name="efrata_permintaan_id" class="col-sm-4 control-label"> User Peminta </label>';
                        dropdown = dropdown + '<select class="form-control" id="efrata_permintaan_id" name="efrata_permintaan_id">';
                        ...
                        if(pemasukan_type == "to user")
                        {
                            ...
                            $.each(response.efrata_permintaans, function(index, permintaans) {
                                $('select[name="efrata_permintaan_id"]').append('<option value="'+permintaans.id+'">'+permintaans.username+'</option>')
                            })
            ...
            $("#efrata_budget_id").on('change', function() {
                var efrata_budget_id = $("#efrata_budget_id").val();
                console.log(efrata_budget_id)
                ...
                $.ajax({
                    ...
                    type: "GET",
                    url: "{{ route('efrata_pemasukan.create') }}",
                    data:  { 'efrata_budget_id' : efrata_budget_id },
                    dataType: "json",
                    beforeSend : function()
                    {
                        console.log(efrata_budget_id);
            ...
            $(document).on('click', '.create', function (e) {
                ...
                var data = {
                    ...
                    // 'status': $('#status').val(),
                    'efrata_budget_id': $('#efrata_budget_id').val(),
                ...
                $.ajax({
                    type : "POST",
                    url : "{{ route('efrata_pemasukan.store') }}",
                    data : data,
                    dataType : "json",
                    success : function(response){
        ...
        $(document).on('click', '.editPemasukan', function(e) {
            ...
            $.ajax({
                type: "GET",
                url: "{{ route('efrata_pemasukan.index') }}" + '/' + id + '/edit',
                dataType: "json",
                success: function (response) {
                    ...
                    else
                    {
                        ...
                        // });
                        $("#id").val(id);
                        $('#updatePemasukan').find('#date').val(response.efrata_pemasukans.date);
                        $('#updatePemasukan').find('#pemasukan_type').val(response.efrata_pemasukans.pemasukan_type);
                        $('#updatePemasukan').find('#ef_uid').val(response.efrata_pemasukans.ef_uid);
                        // $('#updatePemasukan').find('#status').val(response.pemasukans.status);

                        $('#updatePemasukan').find('#efrata_budget_id').val(response.efrata_budgets.budget_id);
                        var option_budget = '<option value = "'+response.efrata_budgets.id+'" selected> --- '+response.efrata_budgets.budget_id+' --- </option>'
                        $('#updatePemasukan').find('select[name="efrata_budget_id"]').append(option_budget);
                        ...
                        if(!response.efrata_pemasukans.efrata_permintaan_id)
                        {
                            showForm.hide();
                        }
                        else
                        {
                            $('#updatePemasukan').find('#efrata_permintaan_id').val(response.efrata_permintaans.username);
                            var option_permintaan = '<option value = "'+response.efrata_permintaans.id+'" selected="selected"> --- '+response.efrata_permintaans.username+' --- </option>'
                            $('select[name="efrata_permintaan_id"]').append(option_permintaan);
                        }

                        //option yang bisa dipilih
                        $.each(response.efrata_permintaansAll, function(index, permintaans) {
                            $('select[name="efrata_permintaan_id"]').append('<option value="'+permintaans.id+'">'+permintaans.username+'</option>')
                        })
                        ...
                        if(response.efrata_pemasukans.pemasukan_type == "to user")
                        {
                            showForm_supplier.show();
                            showForm_manufacture.show();
                            showForm.show();
                        }
                        else if(response.efrata_pemasukans.pemasukan_type == "to it stock")
                        {
                            showForm.hide();
                            showForm_supplier.show();
                            showForm_manufacture.show();
                        }
                        else if(response.efrata_pemasukans.pemasukan_type == "replacement")
                        {
                        ...
                        $('#updatePemasukan').find('#pemasukan_type').on('change', function() {
                            ...
                            {
                                if(!response.efrata_pemasukans.unit_id)
                                {
                                ...
                                else if(response.efrata_pemasukans.unit_id)
                                {
                                    $('#updatePemasukan').find('#unit_id').val(response.units.unit_name);
                                    var option_unit = '<option value = "'+response.units.id+'" selected="selected"> --- '+response.units.unit_name+' --- </option>'
                                    $('#updatePemasukan').find('select[name="unit_id"]').append(option_unit);
                        ...
                        category_type = '<label name="category_type" class="col-sm-4 control-label"> Tipe Kategori </label>';
                        category_type = category_type + '<div class="col-sm-12">';
                        category_type = category_type + '<input type="text" class="form-control" id="category_type" name="category_type" value="'+response.efrata_pemasukans.category_type+'" maxlength="50" readonly>';
                        category_type = category_type + '</div>';

                        category_name = '<label name="category_name" class="col-sm-4 control-label"> Nama Barang </label>';
                        category_name = category_name + '<div class="col-sm-12">';
                        category_name = category_name + '<input type="text" class="form-control" id="category_name" name="category_name" value="'+response.efrata_pemasukans.category_name+'" maxlength="50" readonly>';
                        category_name = category_name + '</div>';

                        barcode = '<label name="barcode" class="col-sm-4 control-label"> Barcode </label>';
                        barcode = barcode + '<div class="col-sm-12">';
                        barcode = barcode + '<input type="text" class="form-control" id="barcode" name="barcode" value="'+response.efrata_pemasukans.barcode+'" maxlength="50" required>';
                        barcode = barcode + '</div>';
                        ...
                        var merk = response.efrata_pemasukans.merk ? response.efrata_pemasukans.merk : " " ;
                        var motherboard = response.efrata_pemasukans.motherboard ? response.efrata_pemasukans.motherboard : " " ;
                        var processor = response.efrata_pemasukans.processor ? response.efrata_pemasukans.processor : " " ;
                        var casing = response.efrata_pemasukans.casing ? response.efrata_pemasukans.casing : " " ;
                        var harddisk_slot_1 = response.efrata_pemasukans.harddisk_slot_1 ? response.efrata_pemasukans.harddisk_slot_1 : " " ;
                        var harddisk_slot_2 = response.efrata_pemasukans.harddisk_slot_2 ? response.efrata_pemasukans.harddisk_slot_2 : " " ;
                        var ram_slot_1 = response.efrata_pemasukans.ram_slot_1 ? response.efrata_pemasukans.ram_slot_1 : " " ;
                        var ram_slot_2 = response.efrata_pemasukans.ram_slot_2 ? response.efrata_pemasukans.ram_slot_2 : " " ;
                        var fan_processor = response.efrata_pemasukans.fan_processor ? response.efrata_pemasukans.fan_processor : " " ;
                        var dvd_internal = response.efrata_pemasukans.dvd_internal ? response.efrata_pemasukans.dvd_internal : " " ;
                        var power_supply = response.efrata_pemasukans.power_supply ? response.efrata_pemasukans.power_supply : " " ;
                        var id_seri = response.efrata_pemasukans.id_seri ? response.efrata_pemasukans.id_seri : " " ;
                        var no_seri = response.efrata_pemasukans.no_seri ? response.efrata_pemasukans.no_seri : " " ;

                        if(response.efrata_pemasukans.category_name == "Laptop" || response.efrata_pemasukans.category_name == "PC")
                        {
                            spesifikasi = '<label name="spesifikasi" class="col-sm-12 control-label"> Spesifikasi Barang </label>';
                        ...
                        else if(response.efrata_pemasukans.category_name == "Scanner" || response.efrata_pemasukans.category_name == "Printer")
                        {
                            spesifikasi = '<label name="spesifikasi" class="col-sm-12 control-label"> Spesifikasi Barang </label>';
        ...
        $(document).on('click', '.update', function(e) {
            ...
            var formData = {
                ef_uid: $('#updatePemasukanForm').find('#ef_uid').val(),
                date: $('#updatePemasukanForm').find('#date').val(),
                pemasukan_type: $('#updatePemasukanForm').find('#pemasukan_type').val(),
                efrata_permintaan_id: $('#updatePemasukanForm').find('#efrata_permintaan_id').val(),
                // 'status': $('#updatePemasukanForm').find('#status').val(),
                efrata_budget_id: $('#updatePemasukanForm').find('#efrata_budget_id').val(),
