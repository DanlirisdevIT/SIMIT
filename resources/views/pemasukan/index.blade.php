...
@section('content')
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <ul id="saveForm_errList"></ul>
                        ...
                        <div class="form-group">
                            <label name="budget_id" class="col-sm-4 control-label"> Budget </label>
    ...
    <script type="text/javascript">
                ...
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        $('#pemasukan_type').on('change', function() {
                            ...
                            dropdown = '<label name="permintaan_id" class="col-sm-4 control-label"> User Peminta </label>';
                            dropdown = dropdown + '<select class="form-control" id="permintaan_id" name="permintaan_id">';
                            ...
                            if(pemasukan_type == "to user")
                            {
                                ...
                                $.each(response.permintaans, function(index, permintaans) {
                                    $('select[name="permintaan_id"]').append('<option value="'+permintaans.id+'">'+permintaans.username+'</option>')
                                })
        ...
        $(document).on('click', '.editPemasukan', function(e) {
            ...
            $.ajax({
                type: "GET",
                url: "{{ route('pemasukan.index') }}" + '/' + id + '/edit',
                dataType: "json",
                success: function (response) {
                    ...
                    else
                    {
                        ...
                        $('#updatePemasukan').find('#date').val(response.getDate);
                        $('#updatePemasukan').find('#pemasukan_type').val(response.pemasukans.pemasukan_type);
                        // $('#updatePemasukan').find('#status').val(response.pemasukans.status);

                        $('#updatePemasukan').find('#budget_id').val(response.budgets.id);
                        var option_budget = '<option value = "'+response.budgets.id+'" selected> --- '+response.budgets.id+' --- </option>'
                        $('#updatePemasukan').find('select[name="budget_id"]').append(option_budget);
                        ...
                        // var permintaan_id = response.permintaans.id ? response.permintaans.id : " " ;
                        // var permintaan_username = response.permintaans.username ? response.permintaans.username : " " ;

                        if(!response.pemasukans.permintaan_id)
                        {
                            showForm.hide();
                        }
                        else
                        {
                            $('#updatePemasukan').find('#permintaan_id').val(response.permintaans.username);
                            var option_permintaan = '<option value = "'+response.permintaans.id+'" selected="selected"> --- '+response.permintaans.username+' --- </option>'
                            $('select[name="permintaan_id"]').append(option_permintaan);
                        }

                        //option yang bisa dipilih
                        $.each(response.permintaansAll, function(index, permintaans) {
                            $('select[name="permintaan_id"]').append('<option value="'+permintaans.id+'">'+permintaans.username+'</option>')
                        })
                        ...
                        if(response.pemasukans.pemasukan_type == "to user")
                        {
                            showForm_supplier.show();
                            showForm_manufacture.show();
                            showForm.show();
                        }
                        else if(response.pemasukans.pemasukan_type == "to it stock")
                        {
                            showForm.hide();
                            showForm_supplier.show();
                            showForm_manufacture.show();
                        }
                        else if(response.pemasukans.pemasukan_type == "replacement")
                        {
                        ...
                        $('#updatePemasukan').find('#pemasukan_type').on('change', function() {
                            ...
                            {
                                if(!response.pemasukans.unit_id)
                                {
                                ...
                                else if(response.pemasukans.unit_id)
                                {
                                    $('#updatePemasukan').find('#unit_id').val(response.units.unit_name);
                                    var option_unit = '<option value = "'+response.units.id+'" selected="selected"> --- '+response.units.unit_name+' --- </option>'
                                    $('#updatePemasukan').find('select[name="unit_id"]').append(option_unit);
                        ...
                        category_type = '<label name="category_type" class="col-sm-4 control-label"> Tipe Kategori </label>';
                        category_type = category_type + '<div class="col-sm-12">';
                        category_type = category_type + '<input type="text" class="form-control" id="category_type" name="category_type" value="'+response.pemasukans.category_type+'" maxlength="50" readonly>';
                        category_type = category_type + '</div>';

                        category_name = '<label name="category_name" class="col-sm-4 control-label"> Nama Barang </label>';
                        category_name = category_name + '<div class="col-sm-12">';
                        category_name = category_name + '<input type="text" class="form-control" id="category_name" name="category_name" value="'+response.pemasukans.category_name+'" maxlength="50" readonly>';
                        category_name = category_name + '</div>';

                        barcode = '<label name="barcode" class="col-sm-4 control-label"> Barcode </label>';
                        barcode = barcode + '<div class="col-sm-12">';
                        barcode = barcode + '<input type="text" class="form-control" id="barcode" name="barcode" value="'+response.pemasukans.barcode+'" maxlength="50" required>';
                        barcode = barcode + '</div>';
                        ...
                        var merk = response.pemasukans.merk ? response.pemasukans.merk : " " ;
                        var motherboard = response.pemasukans.motherboard ? response.pemasukans.motherboard : " " ;
                        var processor = response.pemasukans.processor ? response.pemasukans.processor : " " ;
                        var casing = response.pemasukans.casing ? response.pemasukans.casing : " " ;
                        var harddisk_slot_1 = response.pemasukans.harddisk_slot_1 ? response.pemasukans.harddisk_slot_1 : " " ;
                        var harddisk_slot_2 = response.pemasukans.harddisk_slot_2 ? response.pemasukans.harddisk_slot_2 : " " ;
                        var ram_slot_1 = response.pemasukans.ram_slot_1 ? response.pemasukans.ram_slot_1 : " " ;
                        var ram_slot_2 = response.pemasukans.ram_slot_2 ? response.pemasukans.ram_slot_2 : " " ;
                        var fan_processor = response.pemasukans.fan_processor ? response.pemasukans.fan_processor : " " ;
                        var dvd_internal = response.pemasukans.dvd_internal ? response.pemasukans.dvd_internal : " " ;
                        var power_supply = response.pemasukans.power_supply ? response.pemasukans.power_supply : " " ;
                        var id_seri = response.pemasukans.id_seri ? response.pemasukans.id_seri : " " ;
                        var no_seri = response.pemasukans.no_seri ? response.pemasukans.no_seri : " " ;

                        if(response.pemasukans.category_name == "Laptop" || response.pemasukans.category_name == "PC")
                        {
                            spesifikasi = '<label name="spesifikasi" class="col-sm-12 control-label"> Spesifikasi Barang </label>';
                        ...
                        else if(response.pemasukans.category_name == "Scanner" || response.pemasukans.category_name == "Printer")
                        {
                            spesifikasi = '<label name="spesifikasi" class="col-sm-12 control-label"> Spesifikasi Barang </label>';
        ...
        $(document).on('click', '.update', function(e) {
            ...
            var formData = {
                ...
                'permintaan_id': $('#updatePemasukanForm').find('#permintaan_id').val(),
                // 'status': $('#updatePemasukanForm').find('#status').val(),
                'budget_id': $('#updatePemasukanForm').find('#budget_id').val(),
