            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="ag_permintaan_id" name="ag_permintaan_id">
                        ...
                        @foreach ($ag_permintaans as $permintaan)
    ...
    <script type="text/javascript">
        $(function() {
            ...
            var table = $('.datatables').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('ag_budget.index') }}",
                method: 'GET',
                columns: [
                    {data: 'DT_RowIndex', name: 'DT_RowIndex', width: '5%'},
                    {data: 'date', name: 'date', width: '15%'},
                    {data: 'budget_id', name: 'budget_id', width: '15%'},
                    {data: 'ag_permintaans.username', name: 'ag_permintaans.username', width: '15%'},
        ...
        $(document).ready(function () {
            ...
            $("#ag_permintaan_id").on('change', function() {
                    ...
                    $.ajax({
                        ...
                        type: "GET",
                        url: "{{ route('ag_budget.create') }}",
                        data:  { 'ag_permintaan_id' : ag_permintaan_id },
                        dataType: "json",
                        beforeSend : function()
                        {
                            console.log(ag_permintaan_id);
                        ...
                        success: function(response){
                            ...
                            $('#quantity').val(response.ag_permintaans.quantity);
                            }
                        }
                    })
            })
            ...
            $('.reset').click(function (){
                $('#date').val("")
                $('#ag_permintaan_id').val($('#ag_permintaan_id').data("default_value"))
        ...
        $(document).on('click', '.editBudget', function (e){
            ...
            $.ajax({
                type: "GET",
                url: "{{ route('ag_budget.index') }}" + '/' + id + '/edit',
                success: function(response){
                    ...
                    {
                        $("#id").val(id);
                        $('#updateBudget').find('#date').val(response.ag_budgets.date);

                        $('#updateBudget').find("#ag_permintaan_id").val(response.ag_permintaans.username);
                        var option_permintaan = '<option value = "'+response.ag_permintaans.id+'" selected> --- '+response.ag_permintaans.username+' --- </option>'
                        $('#updateBudget').find('select[name="ag_permintaan_id"]').append(option_permintaan);

                        $('#updateBudget').find("#group").val("seletedIndex", 0);
                        $('#updateBudget').find("#group").val(response.ag_budgets.group);
                        var option_budget = '<option value = "'+response.ag_budgets.group+'" selected> --- '+response.ag_budgets.group+' --- </option>'
                        ...
                        $('#updateBudget').find("#quantity").val(response.ag_budgets.quantity);
                        $('#updateBudget').find("#unitPrice").val(response.ag_budgets.unitPrice);

                        $('#updateBudget').find('#totalPrice').val(response.ag_budgets.totalPrice);
                        $("#quantity, #unitPrice").keyup(function(){
        ...
        $(document).on('click', '.update', function(e) {
            ...
            var formData = {
                ...
                ag_permintaan_id: $('#updateBudgetForm').find('#ag_permintaan_id').val(),
